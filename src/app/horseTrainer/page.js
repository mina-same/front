  ...
  useEffect(() => {
    const fetchTrainers = async () => {
      try {
        ...
        // Update the trainers state with the fetched data
        setTrainers(result);
      } catch (err) {
        // Handle errors
        console.error('Error fetching horse trainers:', err);
        setError('Failed to load horse trainers. Please try again later.');
      } finally {
      ...
      <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100">
        ...
        <div className="relative h-64 bg-slate-900">
          ...
          <div className="relative h-full flex flex-col items-center justify-center text-white px-4">
            <h1 className="text-4xl md:text-5xl font-bold mb-4">Professional Horse Trainers</h1>
            <p className="text-lg md:text-xl text-center max-w-2xl">
              Find the best horse trainers to enhance your equestrian skills
            </p>
          </div>
        </div>
                ...
                <Card key={index} className="hover:shadow-lg transition-shadow duration-300">
                  ...
                  <CardHeader>
                    <CardTitle className="flex justify-between items-start">
                      <div>
                        <h3 className="text-xl font-semibold">{trainer.name_en}</h3>
                      </div>
                    ...
                    <CardDescription className="flex items-center gap-2 text-gray-600">
                      ...
                      {trainer.country?.name_en}, {trainer.government?.name_en}
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <p className="text-gray-600 mb-4 line-clamp-3">{trainer.about_en}</p>
