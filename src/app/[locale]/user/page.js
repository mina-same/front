    ...
    <div className={`mt-8 flex ${isRTL ? 'flex-row-reverse space-x-reverse' : 'space-x-4'} justify-between`}>
      {step > 1 && (step > 2 || !isEmailVerified) && (
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          className="px-6 py-3 border-2 border-black/10 text-black rounded-md hover:border-black hover:bg-black/5 flex items-center space-x-2"
        >
      ...
      {step < totalSteps ? (
        <motion.button
          ...
          whileHover={{ scale: 1.05, boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}
          whileTap={{ scale: 0.95 }}
          className={`px-6 py-3 rounded-md flex items-center space-x-2 ${isLoading ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-black text-white hover:bg-gray-800'
            }`}
        >
          {isLoading ? (
            <>
        ...
        <motion.button
          ...
          whileHover={{ scale: 1.05, boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}
          whileTap={{ scale: 0.95 }}
          className={`px-6 py-3 rounded-md flex items-center space-x-2 transition-colors duration-200 ${isFormValid && !isLoading
...
export default function SignUpForm() {
  ...
  useEffect(() => {
    const fetchSanityData = async () => {
      try {
        ...
        const productQuery = '*[_type == "product"]{ _id, name_en, name_ar }';
  ...
  const handleProductToggle = (productId) => {
    const currentProducts = formData.supplierDetails.products;
    handleChange('supplierDetails', 'products',
    ...
    switch (currentStep) {
      ...
      case 6:
        if (formData.personalInfo.userType === 'rider') {
          ...
          if (sd.products.length === 0) newErrors.products = t('user:error.productsRequired');
          stepValid = Object.keys(newErrors).length === 0;
    ...
    try {
          ...
          const medCertUploads = await Promise.all(
            rd.medicalCertificates.map(async (file, index) => {
              const fileAsset = await client.assets.upload('file', file, {
                filename: `${userId}-med-cert-${index}`,
              });
      ...
      if (formData.personalInfo.userType === 'suppliers') {
        ...
        userData.supplierDetails = {
          products: sd.products.map(productId => ({
            _type: 'reference',
            _ref: productId,
          })),
        };
      ...
      if (formData.personalInfo.userType === 'educational_services') {
        ...
        if (ed.certifications) {
          const certAsset = await client.assets.upload('file', ed.certifications, {
            filename: `${userId}-educational-cert`,
          });
      ...
      case 1:
        ...
        return (
          <div className="space-y-6">
            ...
            {error && (
              <div className={`p-4 bg-red-50 text-red-700 rounded-lg text-sm ${isRTL ? 'font-arabic text-right' : 'text-left'}`}>
                {error}
              </div>
            )}
            <div className="space-y-4">
              <div>
                <Input
                  ...
                  className={`p-4 rounded-md border border-gray-100 shadow-sm focus:ring-gold focus:border-gold ${isRTL ? 'text-right font-arabic' : 'text-left'}`}
                  dir={isRTL ? 'rtl' : 'ltr'}
                />
                {errors.userName && (
          ...
          <div className="space-y-6">
            ...
            <div>
              <Input
                ...
                className={`p-4 rounded-md border border-gray-100 shadow-sm focus:ring-gold focus:border-gold text-center ${isRTL ? 'font-arabic' : ''}`}
                placeholder="XXXXXX"
                maxLength={6}
                dir={isRTL ? 'rtl' : 'ltr'}
              />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div className="sm:col-span-2">
                ...
                <PhoneInput
                  ...
                  inputClass={`rounded-md border border-gray-100 shadow-sm focus:ring-gold focus:border-gold text-black ${isRTL ? 'text-right font-arabic' : 'text-left'}`}
                ...
                <div className="grid grid-cols-2 gap-2">
                  {['male', 'female'].map(g => (
                    <Button
                      ...
                      className={`p-3 rounded-md ${pi.gender === g ? 'bg-gold/10 border-gold text-gold' : 'border-gray-300 hover:bg-gray-50'
                ...
                <div className="grid grid-cols-2 gap-4">
                  {['rider', 'provider', 'suppliers', 'educational_services'].map(type => (
                    <Button
                      ...
                      className={`p-4 rounded-md ${pi.userType === type ? 'bg-gold/10 border-gold text-gold' : 'border-gray-300 hover:bg-gray-50'
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                ...
                <select
                  ...
                  className={`w-full p-4 rounded-md border border-gray-100 shadow-sm focus:ring-gold focus:border-gold ${isRTL ? 'text-right font-arabic' : 'text-left'}`}
                  dir={isRTL ? 'rtl' : 'ltr'}
                >
          ...
          return (
            <div className="space-y-8 p-6 bg-white rounded-xl shadow-sm">
              <div className={`flex items-center ${isRTL ? 'space-x-reverse' : 'space-x-4'}`}>
                <div className="bg-gradient-to-r from-blue-600 to-blue-800 p-3 rounded-lg shadow-md">
              ...
              <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {[
                      ...
                      <Button
                        ...
                        className={`p-4 rounded-lg transition-all duration-200 hover:scale-105 ${rd.eventTypes.includes(type)
                ...
                <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
                  ...
                  <div className="relative">
                    <Select
                      ...
                      className={`p-4 rounded-lg border border-gray-200 shadow-sm focus:ring-blue-600 focus:border-blue-600 ${isRTL ? 'text-right font-arabic' : 'text-left'} text-base w-full`}
                      dir={isRTL ? 'rtl' : 'ltr'}
                    >
                ...
                <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
                  ...
                  <Input
                    ...
                    className={`p-4 rounded-lg border border-gray-200 shadow-sm focus:ring-blue-600 focus:border-blue-600 ${isRTL ? 'text-right font-arabic' : 'text-left'} text-base`}
                    min="0"
                    dir={isRTL ? 'rtl' : 'ltr'}
                  />
                ...
                <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
                  ...
                  {rd.certifications.map((cert, index) => (
                    <div
                      key={index}
                      className={`flex flex-col sm:flex-row gap-3 mb-4 p-4 bg-white rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-200`}
                    >
                      <Input
                        type="text"
                        ...
                        className={`flex-1 p-4 rounded-lg border border-gray-200 focus:ring-blue-600 focus:border-blue-600 ${isRTL ? 'text-right font-arabic' : 'text-left'} text-base`}
                        placeholder={t('user:certificationName')}
                        dir={isRTL ? 'rtl' : 'ltr'}
                      />
                      <div className={`flex items-center gap-2`}>
                        <label
                          className={`px-4 py-3 bg-blue-50 text-blue-600 rounded-lg cursor-pointer hover:bg-blue-100 transition-colors flex items-center gap-2 whitespace-nowrap`}
                        >
                  ...
                  <Button
                    ...
                    className={`mt-3 px-5 py-3 border-2 border-blue-600 text-blue-600 rounded-lg hover:bg-blue-50 transition-colors flex items-center justify-center gap-2 w-full sm:w-auto`}
                  >
                ...
                <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
                  ...
                  <Textarea
                    ...
                    className={`p-4 rounded-lg border border-gray-200 shadow-sm focus:ring-blue-600 focus:border-blue-600 ${isRTL ? 'text-right font-arabic' : 'text-left'} text-base`}
                    rows={4}
                    placeholder={t('user:describeHealthCondition')}
                    dir={isRTL ? 'rtl' : 'ltr'}
                  />
                </div>
                ...
                <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
                  ...
                  <div className="flex flex-col space-y-3">
                    <label
                      className={`px-5 py-4 bg-blue-50 text-blue-600 rounded-lg cursor-pointer hover:bg-blue-100 transition-colors flex items-center justify-center gap-2 w-full sm:w-auto`}
                    >
                      <Upload className="w-5 h-5" />
            ...
            <div className="space-y-8 p-6 bg-white rounded-xl shadow-sm">
              ...
              <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {products.map(product => (
                    <Button
                      key={product._id}
                      variant={sd.products.includes(product._id) ? 'default' : 'outline'}
                      onClick={() => handleProductToggle(product._id)}
                      className={`p-4 rounded-lg transition-all duration-200 hover:scale-105 ${sd.products.includes(product._id)
                        ? 'bg-blue-600 border-blue-700 text-white shadow-md'
                        : 'border-gray-300 hover:bg-gray-100 text-gray-700'
                        } flex items-center ${isRTL ? 'space-x-reverse space-x-3 font-arabic' : 'space-x-3'}`}
                    >
              ...
              <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
                ...
                <div className="flex items-center gap-3">
                  <label
                    className={`px-5 py-4 bg-blue-50 text-blue-600 rounded-lg cursor-pointer hover:bg-blue-100 transition-colors flex items-center gap-2`}
                  >
                    <Upload className="w-5 h-5" />
          ...
          <div className="max-w-4xl mx-auto">
            ...
            >
              <form onSubmit={(e) => e.preventDefault()} className="space-y-6">
                ...
                {error && (
                  <div
                    className={`p-4 bg-red-50 text-red-700 rounded-lg text-sm ${isRTL ? 'font-arabic text-right' : 'text-left'}`}
                  >
                    {error}
                  </div>
                )}
                ...
                {message && (
                  <div
                    className={`p-4 bg-green-50 text-green-700 rounded-lg text-sm ${isRTL ? 'font-arabic text-right' : 'text-left'}`}
                  >
                    {message}
                  </div>
                )}
