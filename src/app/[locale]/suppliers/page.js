...
const SuppliersPage = () => {
  ...
  useEffect(() => {
    const fetchSuppliers = async () => {
      try {
        const query = `*[_type == "services" && serviceType == "suppliers" && statusAdminApproved == true && isMainService == true] {
          _id,
          name_en,
          name_ar,
          about_en,
        ...
        setError(t('servicesPage:suppliers.errorMessage'));
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className={`min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 ${isArabic ? 'rtl' : 'ltr'}`}>
        <div className="relative h-64 bg-slate-900">
          ...
          <div className="relative h-full flex flex-col items-center justify-center text-white px-4">
            <h1 className="text-4xl md:text-5xl font-bold mb-4">{t('servicesPage:suppliers.heroTitle')}</h1>
            <p className="text-lg md:text-xl text-center max-w-2xl">
              {t('servicesPage:suppliers.heroDescription')}
            </p>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 -mt-8">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder={t('servicesPage:suppliers.searchPlaceholder')}
        ...
        <div className="max-w-7xl mx-auto px-4 py-12">
          {loading ? (
            ...
            <div className="text-center py-12">
              <p className="text-gray-500">{t('servicesPage:suppliers.noSuppliersMessage')}</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredSuppliers.map((supplier, index) => (
                <Card key={index} className="hover:shadow-lg transition-shadow duration-300" onClick={() => route.push(`/services/${supplier._id.replace('drafts.', '')}`)}>
                  <div className="aspect-video w-full overflow-hidden rounded-t-lg">
                    <Image
                      src={supplier.image ? urlFor(supplier.image).url() : '/api/placeholder/400/300'}
                      alt={isArabic ? supplier.name_ar : supplier.name_en}
                  ...
                  <CardHeader>
                    <CardTitle className={`flex justify-between items-start ${isArabic ? 'flex-row-reverse' : ''}`}>
                      <div>
                        <h3 className="text-xl font-semibold">{isArabic ? supplier.name_ar : supplier.name_en}</h3>
                      </div>
                    ...
                    <CardDescription className={`flex items-center gap-2 text-gray-600 ${isArabic ? 'flex-row-reverse' : ''}`}>
                      ...
                      {isArabic ? `${supplier.country?.name_ar}, ${supplier.government?.name_ar}` : `${supplier.country?.name_en}, ${supplier.government?.name_en}`}
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <p className={`text-gray-600 mb-4 line-clamp-3 ${isArabic ? 'text-right' : ''}`}>
                      {isArabic ? supplier.about_ar : supplier.about_en}
                    </p>
                      ...
                      {supplier.links && supplier.links.length > 0 && (
                        <div className={`flex items-center gap-2 text-blue-500 ${isArabic ? 'flex-row-reverse' : ''}`}>
                          ...
                          <a href={supplier.links[0]} target="_blank" rel="noopener noreferrer" className="hover:underline">
                            {t('servicesPage:suppliers.visitWebsite')}
                          </a>
                        </div>
                      )}
                    </div>
                    <div className="mt-4">
                      <p className={`text-gray-600 ${isArabic ? 'text-right' : ''}`}>{supplier.supplierDetails}</p>
                    </div>
                  </CardContent>
                </Card>
              ))}
