...
import { validateStep as validateBasicInfo } from "./CourseBasicInfoStep";
import { validateStep as validateContent } from "./CourseContentStep";
import { validateStep as validateMedia } from "./CourseMediaStep";
  ...
  const handleNext = () => {
    ...
    switch (currentStep) {
      case 1:
        ...
        break;
      case 2:
        isValid = validateContent(formData, t, setErrors);
        break;
      case 3:
      ...
      {isLastStep ? (
        ...
        >
          {isSubmitting ? (
            ...
            <>
              <Save size={16} />
              <span>{t("addCourse:createCourse")}</span>
            </>
          )}
        </Button>
      ) : (
        <Button
          ...
          className="rounded-xl flex items-center gap-2 bg-black hover:bg-black/90 text-white"
        >
          <span>{t("addCourse:nextStep")}</span>
